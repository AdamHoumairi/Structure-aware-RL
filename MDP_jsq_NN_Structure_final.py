...
def build_multi_lambda_dataset(lambda_list,
    ...
    Features:
      [state_bits..., lambda_norm, rho, gamma_norm, total1_norm, total2_norm, S_norm]
    Targets:
    ...
    for lambda_rate in lambda_list:
        ...
        for i, s in enumerate(states):
            ...
            rho = lambda_rate / (N * K * mu_cont + 1e-12)
            gamma = mu_vm / (mu_cont + 1e-12)
            gamma_norm = gamma / (1.0 + gamma)  # in (0,1)

            feat = state_bits + [lambda_norm, rho, gamma_norm, total1_norm, total2_norm, S_norm]
...
class PositiveLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        w = torch.nn.functional.softplus(self.weight_raw)  # ensures w >= 0
        return torch.nn.functional.linear(x, w, self.bias)


class EnvelopeMonotoneDecompQNet(nn.Module):
    """
    Patch 3b network: enforce monotonicity only on the *envelope* (load) features,
    while keeping full expressive power for composition effects.

    We decompose:
        Q1(s) = B(s) + 0.5 * A(s)
        Q2(s) = B(s) - 0.5 * A(s)

    - Baseline B(s) has a monotone branch B_mono(envelope) with non-negative weights,
      plus a small unconstrained B_ctx(context) term where context includes (lambda_norm, rho=lambda/(N*K*mu_cont), and gamma_norm=mu_vm/(mu_vm+mu_cont)). This allows the baseline to adapt when mu_vm != mu_cont, while preserving monotonicity in the envelope.
    - Advantage A(s) is fully unconstrained (captures non-monotone composition effects).
    """
    def __init__(self, input_dim: int, hidden_dim: int = 64, envelope_dim: int = 3, context_dim: int = 3):
        super().__init__()
        self.input_dim = input_dim
        self.envelope_dim = envelope_dim  # last envelope_dim features are envelope
        self.context_dim = context_dim
        # Expected feature order (from build_multi_lambda_dataset):
        #   [state_bits..., lambda_norm, rho, gamma_norm, total1_norm, total2_norm, S_norm]
        # Context = [lambda_norm, rho, gamma_norm] sits immediately before the envelope.

        # Monotone baseline branch on envelope
        self.b1 = PositiveLinear(envelope_dim, hidden_dim)
        self.b2 = PositiveLinear(hidden_dim, hidden_dim)
        self.b3 = PositiveLinear(hidden_dim, 1)

        # Unconstrained lambda baseline term
        self.bl1 = nn.Linear(context_dim, hidden_dim)
        self.bl2 = nn.Linear(hidden_dim, 1)

        # Unconstrained advantage branch on full features
        self.a1 = nn.Linear(input_dim, hidden_dim)
        self.a2 = nn.Linear(hidden_dim, hidden_dim)
        self.a3 = nn.Linear(hidden_dim, 1)

        self.act = nn.ReLU()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        envelope = x[:, -self.envelope_dim:]  # [total1_norm, total2_norm, S_norm]
        context  = x[:, -(self.envelope_dim + self.context_dim):-self.envelope_dim]  # [lambda_norm, rho, gamma_norm]

        # Monotone baseline
        h = self.act(self.b1(envelope))
        h = self.act(self.b2(h))
        B_mono = self.b3(h)

        # Context baseline
        hc = self.act(self.bl1(context))
        B_ctx = self.bl2(hc)

        B = B_mono + B_ctx

        # Advantage
        ha = self.act(self.a1(x))
        ha = self.act(self.a2(ha))
        A = self.a3(ha)
...
def train_multi_lambda_nn(lambda_list,
                          ...
                          verbose=True):
    """
    Train a single EnvelopeMonotoneDecompQNet on multiple lambda values.
    ...
    model = EnvelopeMonotoneDecompQNet(input_dim=input_dim, hidden_dim=hidden_dim)
...
def train_multi_lambda_nn_from_dataset(X_np, Y_np, opt_actions_np, sample_weights_np,
    ...
    model = init_model if init_model is not None else EnvelopeMonotoneDecompQNet(input_dim=input_dim, hidden_dim=hidden_dim)
    optimizer = optim.Adam(model.parameters(), lr=lr)
...
def build_nn_policy_for_lambda_multi(model, states, lambda_rate, lambda_max, mu_vm, mu_cont):
    r"""
    Build a deterministic policy dictionary from a trained multi-$\lambda$ model for a given $\lambda$.

    Feature construction must match the training dataset:
        feat = state_bits + [lambda_norm, rho, gamma_norm, total1_norm, total2_norm, S_norm]
    where:
        lambda_norm = lambda_rate / lambda_max
        rho         = lambda_rate / (N*K*mu_cont)
        gamma_norm  = (mu_vm/mu_cont) / (1 + mu_vm/mu_cont)

    This makes the learned policy sensitive to mismatched rates (mu_vm != mu_cont) without changing
    the envelope-monotonic structure.
    """
    lambda_norm = lambda_rate / lambda_max
    ...
    # Context features (rate-aware)
    rho = lambda_rate / (N * K * mu_cont + 1e-12)
    gamma = mu_vm / (mu_cont + 1e-12)
    gamma_norm = gamma / (1.0 + gamma)
...
def nn_induced_weights_for_lambda(model_nn, states, state_to_index, lambda_rate, lambda_max,
                                 N, K, C_vm, C_cont, mu_vm, mu_cont):
    nn_policy_dict = build_nn_policy_for_lambda_multi(model_nn, states, lambda_rate, lambda_max, mu_vm, mu_cont)
...
def run_for_lambda_sweep():
    ...
    print("Training lambdas:", lambda_values)
    ...
    for lambda_rate in range(lambda_min, lambda_max + 1):
        ...
        nn_policy_dict = build_nn_policy_for_lambda_multi(
            model_nn, states, lambda_rate, lambda_max, mu_vm, mu_cont
        )
